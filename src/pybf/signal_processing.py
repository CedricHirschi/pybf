...
def demodulate_decimate(
    ...
    carrier = 2 * np.exp(-1.0j * 2 * np.pi * f_carrier * time_arr)
...
def interpolate_modulate(
    ...
    # Create complex carrier wave
    carrier = np.exp(1.0j * 2 * np.pi * f_carrier * time_arr)

    # Multiply the interpolated data by the complex carrier wave
    data_out = data_IQ_interp * np.tile(
        carrier.reshape(1, -1), (data_IQ_interp.shape[0], 1)
    )
