...
def calc_propagation_delays(
    ...
    if tx_strat_name[0] == "PW":
        ...
        tx_delays = calc_dist_from_pw_line_to_point_2(
            pw_angles_rad, pixels_coords, elements_coords_x_max
        )
        tx_delays = tx_delays / speed_of_sound

        # Calculate delays from points to elements
        # Output: distance (n_elements x n_points)
        rx_delays = calc_dist_from_point_to_element(elements_coords, pixels_coords)
        ...
        raise ValueError(
            f"Unknown TX strategy: {tx_strat_name[0]}. Only plane waves are supported for now."
        )
...
def calc_dist_from_pw_line_to_point_2(pw_angle, point_coord_x_z, max_x_coord):
    ...
    # Calculate distance
    # ("Wireless_Real-Time_Plane-Wave_Coherent_Compounding_on_an_iPhone_A_Feasibility_Study")
    distance = np.multiply(z_coords, np.cos(pw_angles_local)) + np.multiply(
        (x_0 - x_coords), np.sin(pw_angles_local)
    )
...
def convert_time_to_samples(array_t, f_sampling, start_offset_s, correction_time_s):
    ...
    array_samples = np.rint(
        np.multiply(array_t - start_offset_s + correction_time_s, f_sampling)
    )
