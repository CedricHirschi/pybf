...
def calc_fov_receive_apodization(
    ...
    for n in range(0, n_pixels):
        active_elements = np.logical_and(
            elements_x >= x_aperture_max[n], elements_x <= x_aperture_min[n]
        )
        apod_weights[n, np.where(active_elements)] = hann_win[
            np.count_nonzero(active_elements)
        ]
