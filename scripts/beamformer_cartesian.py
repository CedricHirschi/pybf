...
# Import pybf modules
from pybf.io_interfaces import DataLoader
...
def beamformer_cartesian(
    ...
    rx_delays_samples = convert_time_to_samples(
        rx_delays, f_sampling_proc, start_time, correction_time_shift
    )
    ...
    apod = calc_fov_receive_apodization(
        int(dl.transducer.num_of_elements),
        dl.transducer.elements_coords,
        pixels_coords,
    ...
    def preprocess_data(rf_data, decim_factor, interp_factor):
        ...
        rf_data_IQ = demodulate_decimate(
            rf_data_filt, dl.f_sampling, dl.transducer.f_central_hz, decim_factor
        )
    ...
    # Allocate the data
    das_out = np.zeros(
        (dl.num_of_acq_per_frame, pixels_coords.shape[1]), dtype=np.complex128
    )
    ...
    for j in frames_to_process:
        ...
        for i in acqs_to_process:
            ...
            # Preproces the data
            rf_data_proc, _ = preprocess_data(
                rf_data, decimation_factor, interpolation_factor
            )
        ...
        if save_images_to_hdf5:
            ...
            if save_lri_to_hdf5:
                saver.save_low_res_images(
                    das_out.reshape(-1, image_res[1], image_res[0]),
                    j,
                    low_res_imgs_indices=acqs_to_process,
                )
            saver.save_high_res_image(
                das_out_compound.reshape(image_res[1], image_res[0]), j
            )
        ...
        print(
            ...
            frame_counter,
            "Progress: ",
            "{0:.2f}".format(frame_counter / len(frames_to_process) * 100),
            " %",
        )
    ...
    if save_images_to_hdf5:
        ...
        saver.save_params(
            pixels_coords, np.array(image_res), dl.transducer.elements_coords, dl.fps
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path_to_rf_dataset", type=str, default=" ", help="Path to the dataset file."
    )
    ...
    parser.add_argument(
        "--save_path", type=str, default=" ", help="Path to the save image dataset."
    )
