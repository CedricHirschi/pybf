...
class BFGlobalMVBF(BFCartesianRealTime):
    def __init__(
        ...
        super(BFGlobalMVBF, self).__init__(
            f_sampling,
            tx_strategy,
            transducer_obj,
            decimation_factor,
    ...
    def _delay_and_sum(self, rf_data_in, delays_idx):
        ...
        # Diagonal loading
        R = R + np.identity(channel_reduction) * np.trace(
            R
        )  #  * 1/(700*channel_reduction)
        ...
        das_out = np.sum(
            np.multiply(rf_data[fancy_idx_samples, fancy_idx_channels], weights_mvbf),
            axis=-1,
        )
