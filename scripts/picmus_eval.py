...
from pybf.visualization import log_compress

import numpy as np


class PicmusEval:
    def __init__(self, bf_image, bf_obj):

        self.image = bf_image
        self.pixel_coords = bf_obj._pixels_coords.reshape(
            2, bf_obj._image_res[1], bf_obj._image_res[0]
        )

    def evaluate_FWHM(
        self, points, is_plot=True, plot_name="FWHM_eval", latex_args=None
    ):
        """
        ...
        if is_plot is True:
            ...
            # Image
            fig, ax = plt.subplots()
            ax.imshow(abs_image, cmap=cm.gray, origin="lower")
    ...
    def evaluate_circ_contrast(
        self, cirles, is_plot=True, plot_name="Constrast_cirC", latex_args=None
    ):
        """
        ...
        for i in range(0, cirles.shape[0]):
            ...
            if is_plot is True:
                ax.add_patch(
                    Circle(
                        (plot[1], plot[0]), plot[2], edgecolor="red", fill=False, lw=2
                    )
                )

                ax.add_patch(
                    Circle(
                        (plot[1], plot[0]), plot[3], edgecolor="green", fill=False, lw=2
                    )
                )
    ...
    def evaluate_rect_contrast(
        self, rectangles, is_plot=True, plot_name="Constrast_rect", latex_args=None
    ):
        if is_plot is True:
            if latex_args is not None:
                from matplotlib import rc
    ...
    def _evaluate_rectangles(self, rectangles):
        ...
        outer_rect = np.append(
            outer_rect, self.image[i_y1:i_y1_end, i_x2_end:i_x1_end].flatten()
        )
        outer_rect = np.append(
            outer_rect, self.image[i_y1:i_y2, i_x2:i_x2_end].flatten()
        )
        outer_rect = np.append(
            outer_rect, self.image[i_y2_end:i_y1_end, i_x2:i_x2_end].flatten()
        )
        ...
        CNR = 20 * np.log10(
            (np.abs(outer_mean - inner_mean))
            / (0.5 * np.sqrt(inner_variance + outer_variance))
        )

        return CNR, np.asarray(
            [
                i_x1,
                i_y1,
                (i_x1_end - i_x1),
    ...
    def _evaluate_circ(self, circle):
        ...
        for x_i in range(0, self.pixel_coords.shape[2]):
            for y_i in range(0, self.pixel_coords.shape[1]):
                radius = np.sqrt(
                    (self.pixel_coords[0, y_i, x_i] - x) ** 2
                    + (self.pixel_coords[1, y_i, x_i] - y) ** 2
                )
                # Inner circle
                if radius < r2:
        ...
        center_coords = np.asarray(
            np.unravel_index(diff_matr.argmin(), diff_matr.shape, order="C")
        )
        pixel_width = np.abs(
            self.pixel_coords[0, center_coords[0], center_coords[1]]
            - self.pixel_coords[0, center_coords[0], center_coords[1] + 1]
        )
        radius_px1 = np.round(r1 / pixel_width)
        radius_px2 = np.round(r2 / pixel_width)

        return CNR, np.asarray(
            [center_coords[0], center_coords[1], radius_px1, radius_px2]
        )
    ...
    def _resolution_el(self, point):
        ...
        max_pos = np.asarray(
            np.unravel_index(img_snippet.argmax(), img_snippet.shape, order="C")
        )
        ...
        while i < self.pixel_coords.shape[2] - 1:
            i = i + 1
            x_upper_val = (
                0.25 * np.abs(self.image[max_pos[0], i - 1])
                + 0.5 * np.abs(self.image[max_pos[0], i])
                + 0.25 * np.abs(self.image[max_pos[0], i + 1])
            )
        ...
        while i < self.pixel_coords.shape[1] - 1:
            i = i + 1
            y_upper_val = (
                0.25 * np.abs(self.image[i - 1, max_pos[1]])
                + 0.5 * np.abs(self.image[i, max_pos[1]])
                + 0.25 * np.abs(self.image[i + 1, max_pos[1]])
            )
