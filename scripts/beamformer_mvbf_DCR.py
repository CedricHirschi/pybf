...
class BFMVBFdcr(BFCartesianRealTime):
    ...
    def beamform(self, rf_data, numba_active=False):
        ...
        for i in acqs_to_process:
            ...
            rf_pre_bf_item = self._pre_bf(
                rf_data_in=rf_data_proc_trans,
                delays_idx=delays_samples.reshape(1, delays_samples.shape[0], -1),
            )
    ...
    def _pre_bf(self, rf_data_in, delays_idx):
        ...
        mvbf_data = rf_data[fancy_idx_samples, fancy_idx_channels][
            0, :, self.start_i : self.stop_i
        ]
    ...
    def _dcr_beamform(self, data_array):
        ...
        for i in range(data_array.shape[1]):
            ...
            if self.inv_approx is False:
                R_inv = np.linalg.inv(
                    corr_matrix + np.identity(L) * np.trace(corr_matrix) * 1 / (1 * L)
                )
